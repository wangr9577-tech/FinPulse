...
class RawNewsSchema(BaseModel):
    ...
    sector: str = Field("未分类", description="所属行业板块分类 (raw 阶段仅源级提示，真实分类由 TaggerAgent 落库判定；如 半导体, 国内宏观)")
...
class StructuredNewsSchema(BaseModel):
    ...
    category_type: str = Field("行业板块", description="一级研判分类: 宏观 / 行业板块")
    sub_category: str = Field("其他板块", description="二级细分: 若为宏观则为'国内宏观'/'国外宏观'；若为行业板块则为东财具体行业名称")
    sector: str = Field("其他板块", description="所属行业板块分类 (对齐东财行业分类或国内/国外宏观)")
