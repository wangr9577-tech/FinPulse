    ...
    def analyze_sector(
        ...
        try:
            ...
            if "summary" not in data or not data["summary"]:
                app_logger.error(f"[Analyst Agent] LLM 未返回板块 {sector_name} 的资讯总结")
                raise ValueError(f"[Analyst Agent] LLM 未返回板块 {sector_name} 的资讯总结")

            sent_str = data.get("sentiment_bias", "中性")
            if sent_str not in ["看多", "看空", "中性"]:
                app_logger.error(f"[Analyst Agent] LLM 返回非法情绪偏向枚举: {sent_str}")
                raise ValueError(f"[Analyst Agent] LLM 返回非法情绪偏向枚举: {sent_str}")
