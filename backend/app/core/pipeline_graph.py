...
from app.agents.tagger_agent import TaggerAgent
...
async def node_extract(state: PipelineGraphState) -> PipelineGraphState:
    ...
    if raw_news:
        ...
        tagger = TaggerAgent()
        # 全量覆盖时间窗内抓取的所有新闻，不再按重要性上限截断到 20 条，
        # 确保研报的"资讯分析/板块深度/总评"建立在完整新闻池之上而非抽样。
        # 仅按重要性降序排序，保证高价值新闻优先处理，但每一条都会萃取+打标。
        ordered_news = sorted(raw_news, key=lambda n: n.get("importance", 1), reverse=True)

        # 有界并发打标：仍是"每条真实 LLM、全量覆盖"，只是多线程并行提升吞吐，
        # 避免 24h 全量逐条串行调用大模型导致耗时数小时。asyncio.gather 会保持输入顺序。
        def _build_card(n: Dict[str, Any]) -> Optional[Dict[str, Any]]:
            """同步逻辑：单条新闻 提取+打标+补齐基础字段；失败返回 None 由调用方跳过"""
            try:
                card = extractor.extract(n)
                # 用 mode="json" 序列化，确保 Sentiment 枚举等字段可被 MongoDB 正常落库
                card_dict = card.model_dump(mode="json")

                # 打标：新增 TaggerAgent 为卡片补齐分类字段 (先大分类 -> 后小分类 两阶段)
                try:
                    tag_res = tagger.tag(n, extracted_facts=card.core_facts, entities=card.entities)
                    card_dict.update(tag_res.model_dump(mode="json"))
                except Exception as e_tag:
                    app_logger.warning(f"打标新闻卡片异常 ({e_tag})，该条保留为未分类")
                    card_dict.setdefault("category_type", "行业板块")
                    card_dict.setdefault("sector", "其他板块")
                    card_dict.setdefault("sub_category", "其他板块")
                    card_dict.setdefault("event_type", "其他")

                # 补齐结构化卡片基础字段 (来自 raw 新闻，用于按板块/来源检索与落库)
                card_dict["raw_id"] = n.get("news_id", "")
                card_dict.setdefault("source", n.get("source", "资讯源"))
                card_dict.setdefault("title", n.get("title") or "")
                card_dict.setdefault("publish_time", n.get("publish_time"))
                return card_dict
            except Exception as e:
                app_logger.warning(f"抽取新闻卡片异常 ({e})，跳过该条")
                return None

        # 有界并发（同时占用的 LLM 线程数，兼顾吞吐与 API 限流）
        CONCURRENCY = 12
        sem = asyncio.Semaphore(CONCURRENCY)

        async def _run_one(n):
            async with sem:
                return await asyncio.to_thread(_build_card, n)

        results = await asyncio.gather(*(_run_one(n) for n in ordered_news))
