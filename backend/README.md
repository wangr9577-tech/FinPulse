...
- **两库分离 + 全量 LLM 真实打标 (`Two-Tier Store & True LLM Tagging`)**：抓取层 `raw_news_collection` 仅作为流水线输入/暂存（其 `sector` 统一为 `未分类`，仅保留源级提示，**移除一切手工硬编码兜底**）；真实板块归属由 `TaggerAgent` 逐条调用 LLM 两阶段判定（先大类 `宏观/行业板块`，再细分 `国内·国外宏观` 或东财 86 个官方行业），结果权威落库至 `structured_news_collection`。所有展示端**只读结构化库**。
- **早停熔断机制 (`Early-Exit Short-Circuit`)**：倒序解析快讯，一旦发布时间超出指定窗口 (由 `settings.REPORT_HOURS_BACK` 全局控制) 自动切断网络请求，同时支持 Google News RSS 与 RSSHub 多镜像节点网络容灾。
...
- **配置驱动架构 (Env-Driven & Fail-Fast)**：移除所有硬编码参数默认值，强制从 `.env` 读取配置并启用 `override=True`；全链路（ExtractorAgent、AnalystAgent、SynthesizerAgent、AuditorAgent、ReportValidator 与 Playwright PDF 引擎）彻底移除所有 Heuristic 降级/兜底硬规则与内存兜底标记，遇到异常即严格抛错并记录 App Logger。
...
- **node_extract 有界并发打标**：对 24h 时间窗内**全量**新闻逐条真实 LLM 打标（不做重要性截断抽样），通过 `asyncio.Semaphore` + `asyncio.to_thread` 做 **12 路有界并发**，将全量打标从串行数小时压缩到十几分钟。
- **Extractor Agent**：对海量资讯执行关联实体抽离、事实提取与情绪标注（仅负责内容提炼，不涉及板块归属；板块归属由 TaggerAgent 负责）。
- **Tagger Agent**：两阶段 LLM 真实分类打标——Stage1 先判 `category_type`（`宏观`/`行业板块`），严格区分「单只个股/单一板块成交额→归其所属东财行业（event_type 一般为产业动态/其他）」与「全市场/宏观资金面（两市总量、央行动作、DR007/SHIBOR、社融、M2、LPR、北向全市场、两融余额）→ `国内宏观·市场流动性`」；Stage2 再细分 `sub_category`/`sector`（国内·国外宏观 或 东财 86 官方行业）。全量覆盖时间窗内每条新闻，**绝不使用手工硬编码标签兜底**。
- **SectorGrouper 板块分类分组**：彻底取消聚类算法，依据 `structured_news_collection` 卡片原生 `sector` 分类标签进行高效字典 `Group-By` 分组，无缝兼容任意新增行业。
...
`.env` 常用参数说明 (完整键名清单见 `backend/.env.example`，与 `backend/.env` 对齐)：
```ini
# 1. LLM 配置 (全系统统一 Agent 模型)
...
1. **全量依赖统一声明 (`Dependency Management`)**：
   全部 Python 依赖（pydantic、akshare、pandas、fastapi、motor、langchain/langgraph、loguru、tenacity、playwright 等 33 项）均声明于 `[project] dependencies`，`requirements.txt` 仅保留为兼容旧安装习惯的冗余说明文件。
2. **自动包发现 (`Package Finding`)**：
   配置 `[tool.setuptools.packages.find]` 显式包含 `app` 与 `scripts` 源码目录，无需手动维护包列表。
3. **可编辑模式一键安装 (`Editable Installation`)**：
   在 `backend` 目录下执行 `pip install -e .` 即可一次性安装全部依赖并注册为本地可编辑包，彻底解决跨目录脚本（如根目录下的 `daily_scheduler_7am.py` 或 `scripts/` 子目录下的测试脚本）因找不到包路径而引发的 `ModuleNotFoundError: No module named 'app'` 导入异常。
4. **跨平台兼容与构建隔离**：
