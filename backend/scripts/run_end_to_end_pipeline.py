...
def run_end_to_end_pipeline(hours_back: Optional[float] = None):
    ...
    print("=" * 80)
    print(" [主流程] 智能投研信息引擎 - 全自动化一键运行主入口 (End-to-End Master Pipeline)")
    print("=" * 80)
    ...
    # -------------------------------------------------------------------------
    # STAGE 1: 资讯与 35 项指标多源数据抓取 (并发并行模式)
    # -------------------------------------------------------------------------
    print(f"[STAGE 1/5] 并发启动数据爬取引擎 (28大媒体快讯 + 择时35项指标, 时间窗口: 过去 {hours_back} 小时)...")
    s1_start = time.time()
    ...
    try:
        ...
        async def _fetch_and_store_news():
            app_logger.info(f"[STAGE 1.1] 正在并发抓取全量 28 大媒体高频新闻快讯 (过去 {hours_back}h)...")
            news_fetcher = FlashNewsFetcher()
            fetched_news = await news_fetcher.fetch_all_flash_news()
            app_logger.info(f"[STAGE 1.1] 新闻抓取完成！共获取 {len(fetched_news)} 条 {hours_back}h 内增量资讯。")
        ...
        def _run_timing_crawler():
            app_logger.info("[STAGE 1.2] 正在并发调度择时指标多源爬虫 (AKShare + 官方 API)...")
            ...
            try:
                ...
                app_logger.warning("[STAGE 1.2] 爬虫调度达 20s 限制，使用当前最新可得数据平滑进入后续计算。")
                class _Proc:
                    returncode = 0
                    stdout = "Timeout fallback"
                    stderr = ""
                return _Proc()

        async def _run_stage1_parallel():
            task_news = asyncio.create_task(_fetch_and_store_news())
            task_crawler = asyncio.to_thread(_run_timing_crawler)
            return await asyncio.gather(task_news, task_crawler)

        raw_news_dicts, proc = asyncio.run(_run_stage1_parallel())

        if proc.returncode == 0:
            app_logger.info("[STAGE 1.2] 择时 35 项指标数据更新成功！")
        else:
            app_logger.error(f"[STAGE 1.2] 择时爬虫运行失败 (退出码: {proc.returncode}): {proc.stderr or proc.stdout}")
            raise RuntimeError(f"[STAGE 1.2 异常中断] 择时爬虫运行失败，退出码: {proc.returncode}")

    except Exception as e_s1:
        app_logger.error(f"[STAGE 1] 数据并发抓取异常: {e_s1}")
        raise RuntimeError(f"[STAGE 1 异常中断] 数据并发抓取失败: {e_s1}") from e_s1

    s1_elapsed = time.time() - s1_start
    print(f"   └─ STAGE 1 完成 (并行抓取耗时: {s1_elapsed:.2f} 秒)\n")
    ...
    try:
        ...
        generate_all_hexagon_charts()
        app_logger.info("[STAGE 3.3] 择时 35 项量化指标折线图与六维度雷达图全部高清渲染落盘！")
    except Exception as e_plot:
        app_logger.warning(f"[STAGE 3.3] 择时图表渲染提示: {e_plot}")

    s3_elapsed = time.time() - s3_start
    print(f"   └─ STAGE 3 完成 (含算子计算/数据库落盘/图表渲染)，耗时: {s3_elapsed:.2f} 秒\n")
    ...
    try:
        ...
        print("=" * 80)
        print(f"  累计总耗时: {total_elapsed:.2f} 秒 (数据抓取: {s1_elapsed:.1f}s | 择时计算: {s2_elapsed:.1f}s | 入库: {s3_elapsed:.1f}s | AI/PDF: {s4_elapsed:.1f}s)")
        print("\n  核心产出文件路径汇总:")
